...
| **H!RE - Match Resume** | Webhook `/match-resume` | ✅ Core | Powers "✨ Auto Find Best Job Position". PDF/OCR → **LLM router** (Groq `gpt-oss-120b`, temp 0) picks the best-fitting requirement by *actual work performed*, returns `best_requirement_id` (Django then runs the full Evaluate on that pick). Replaced the old SBERT cosine picker, which mis-routed by keyword/vocabulary overlap (e.g. an IT resume → Accountant). Keyword-overlap fallback if Groq is unavailable. Uses the same `groqApi` credential as Evaluate. |
